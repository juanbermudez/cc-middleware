...
```
~/.claude/projects/       Session Watcher          WebSocket Clients
  *.jsonl files    -----> (chokidar + poll) -----> session:discovered
                                |                  session:updated
                                |                  session:removed
                                v
                          Auto-Indexer
...
~/.claude/                Config Watcher           WebSocket Clients
  settings.json    -----> (chokidar + poll) -----> config:changed
  .claude.json                                     config:mcp-changed
  agents/*.md                                      config:agent-changed
  skills/*/SKILL.md                                config:skill-changed
  rules/*.md                                       config:rule-changed
  plugins/                                         config:plugin-changed
  memory/                                          config:memory-changed
  teams/                                           team:created
  tasks/                                           team:updated
                                                   team:task-updated
```
