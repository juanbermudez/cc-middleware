...
<img className="diagram-expandable" data-zoom-disabled="true" data-zoomable="false" src="/images/diagrams/concepts-sessions-0.svg" alt="concepts-sessions-0" style={{ width: '100%', cursor: 'pointer' }} />
...
The discovery module wraps the Agent SDK's `listSessions()` to enumerate sessions across all projects or within a specific directory. The HTTP catalog layer then overlays that raw filesystem discovery with indexed metadata, subagent relationships, and team membership when the SQLite store is available.
...
Each discovered session starts with metadata from Claude Code's `SDKSessionInfo`:
...
The enriched catalog returned by `GET /api/v1/sessions` also adds:

| Field | Description |
|-------|-------------|
| `id` | Convenience alias for `sessionId` |
| `project` | Basename of the session working directory |
| `directoryPath` / `directoryName` | Exact `cwd` grouping used by the directory explorer |
| `indexed` | Whether the session is present in the SQLite index |
| `messageCount` | Indexed message count, when known |
| `metadata` | Registered session metadata entries |
| `lineage` | Subagent/team relationship summary for the root session |
...
Two related HTTP surfaces intentionally answer different questions:

- `GET /api/v1/sessions` answers "what sessions exist on disk right now?" and enriches them with indexed metadata when available.
- `GET /api/v1/search` answers "what sessions and messages are already indexed in SQLite and searchable?"

The playground makes this distinction explicit. It is normal for a session to appear in the catalog before it appears in search if the auto-indexer has not processed it yet.

For explorer-style grouping, `GET /api/v1/sessions/directories` returns the same catalog grouped by exact working directory, including indexed/unindexed counts and relationship summaries.
...
<Tabs>
  <Tab title="Single-Turn">
    ...
    // result.sessionId, result.result, result.totalCostUsd, result.durationMs
    ```
...
Sessions can carry typed metadata definitions and per-session values in the SQLite store. This is the mechanism the playground uses for metadata-aware filtering and its metadata lab.

<CodeGroup>
```bash Definitions
curl http://127.0.0.1:3000/api/v1/sessions/metadata/definitions
```

```bash Set a value
curl -X PUT http://127.0.0.1:3000/api/v1/sessions/{id}/metadata \
  -H "Content-Type: application/json" \
  -d '{ "key": "workflow", "value": "delivery-review" }'
```

```bash List a session's metadata
curl http://127.0.0.1:3000/api/v1/sessions/{id}/metadata
```
</CodeGroup>
